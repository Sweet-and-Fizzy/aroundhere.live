...
<template>
  <UCard class="hover:shadow-lg transition-all duration-200 border-l-4 border-l-primary-500 overflow-hidden" :ui="{ body: displayImage ? 'p-0 sm:p-0' : undefined }">
    ...
    <div class="flex flex-col md:flex-row">
      ...
      <div v-if="displayImage" class="flex-shrink-0 w-full md:w-48 lg:w-56 overflow-hidden bg-gray-100" :class="isVenueLogo ? 'aspect-video md:aspect-square flex items-center justify-center p-4' : 'aspect-square'">
        <img
          :src="displayImage"
          :alt="event.title"
          :class="isVenueLogo ? 'max-w-full max-h-full object-contain' : 'w-full h-full object-cover object-center'"
          loading="lazy"
        />
      </div>

      <!-- Date Block (only show if no image) - Horizontal on mobile, vertical on md+ -->
      <div v-else class="flex md:flex-col items-center md:items-stretch gap-3 md:gap-0 flex-shrink-0 md:w-20 md:text-center py-1">
        <div class="bg-primary-50 rounded-lg px-4 py-2 md:px-2 md:py-3 flex md:flex-col items-center md:items-stretch gap-2 md:gap-0">
          <div class="text-xs font-medium text-primary-600 uppercase tracking-wide">{{ formattedDate.weekday }}</div>
          <div class="text-2xl md:text-3xl font-bold text-primary-700">{{ formattedDate.day }}</div>
          <div class="text-sm font-medium text-primary-600 uppercase">{{ formattedDate.month }}</div>
        </div>
        <div class="flex md:flex-col items-center gap-2 md:gap-0 md:mt-2">
          <div class="text-sm font-medium text-gray-700">{{ formattedTime }}</div>
          <div v-if="doorsTime" class="text-xs text-gray-500">Doors {{ doorsTime }}</div>
        </div>
      </div>

      <!-- Event Details - add padding when image is present since card body has no padding -->
      <div class="flex-1 min-w-0" :class="displayImage ? 'p-4 md:py-4 md:pr-4 md:pl-4 lg:pl-6' : 'py-1'">
        <!-- Date/Time row -->
        <div class="flex flex-wrap items-center gap-1 sm:gap-2 text-sm text-gray-600 mb-1">
          ...
          <span v-if="doorsTime" class="text-gray-400 hidden sm:inline">(doors {{ doorsTime }})</span>
        </div>

        <NuxtLink :to="`/events/${event.slug}`" class="block group">
          <h3 class="font-bold text-base sm:text-lg text-gray-900 group-hover:text-primary-600 transition-colors line-clamp-2">
            {{ event.title }}
          </h3>
        </NuxtLink>

        <div v-if="event.venue" class="mt-1">
          <NuxtLink
          ...
          >
            ...
            <span v-if="event.venue.city" class="text-gray-400 hidden sm:inline">
              - {{ event.venue.city }}
            </span>
          </NuxtLink>
        </div>
        ...
        <div class="flex flex-wrap items-center gap-1.5 sm:gap-2 mt-2">
          ...
          <UBadge
            ...
            size="sm"
          >
            {{ event.ageRestriction.replace('_', ' ').replace('PLUS', '+') }}
          </UBadge>
          <UBadge
            ...
            size="sm"
          >
            <UIcon name="i-heroicons-musical-note" class="w-3 h-3 mr-1" />
            {{ genre }}
          </UBadge>
        </div>
        ...
        <div v-if="event.description" class="mt-3">
          <div class="text-sm text-gray-600">
            <p v-if="!expanded" class="whitespace-pre-line">{{ truncatedDescription }}</p>
      ...
      <div class="hidden lg:flex flex-shrink-0 flex-col justify-center gap-2" :class="event.imageUrl ? 'p-4 pl-0' : ''">
        <UButton
          v-if="event.ticketUrl"
          :to="event.ticketUrl"
          target="_blank"
