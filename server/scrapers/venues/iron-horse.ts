...
export const ironHorseConfig: ScraperConfig = {
  ...
  url: 'https://ironhorse.org/calendar',
  enabled: true,
...
export class IronHorseScraper extends PlaywrightScraper {
  ...
  protected async parseEvents(html: string): Promise<ScrapedEvent[]> {
    ...
    // Iron Horse uses a third-party calendar widget (eapp-events-calendar)
    ...
    // Fallback to old selectors if widget not found
    if (events.length === 0) {
      const oldSelectors = ['.eventlist-event', '.summary-item', '.collection-item']
    ...
    // Try script data as last resort
    if (events.length === 0) {
  ...
  private extractLdJsonEvents($: ReturnType<typeof this.$>): Map<string, { description?: string; image?: string }> {
    ...
    $('script[type="application/ld+json"]').each((_, el) => {
      ...
      try {
        ...
        if (data['@type'] === 'Event') {
          const name = this.decodeHtmlEntities(data.name || '')
          if (name) {
            eventData.set(name.toLowerCase(), {
              description: data.description ? this.decodeHtmlEntities(data.description) : undefined,
  ...
  private parseEappEventElement(
    ...
    try {
      const title = this.cleanText(
        $el.find('.eapp-events-calendar-masonry-item-name').text()
      )
        ...
        if (jsonLdMatch) {
          try {
            ...
            if (!description && schemaData.description) {
              description = this.decodeHtmlEntities(schemaData.description)
            }
      ...
      console.error('[Iron Horse] Error parsing eapp event element:', error)
      return null
    }
  }
  ...
  private parseEventElement(
    ...
    try {
      ...
      const title = this.cleanText(
        $el.find('.eventlist-title, .summary-title, .event-title, h2, h3').first().text()
      )
      ...
      // Try to find date
      const dateText = this.cleanText(
        $el.find('.eventlist-meta-date, .summary-metadata-item--date, .event-date, time').first().text() ||
      ...
      // Try to find time
      const timeText = this.cleanText(
        $el.find('.eventlist-meta-time, .event-time').first().text() || ''
      )

      const startsAt = this.parseDate(dateText, timeText)
      if (!startsAt) {
        console.log(`[Iron Horse] Could not parse date for: ${title}`)
        return null
      }

      // Extract link
      const link = $el.find('a').first().attr('href') || ''
      const sourceUrl = link.startsWith('http') ? link : `https://ironhorse.org${link}`

      // Try to find price
      const priceText = $el.find('.eventlist-meta-price, .event-price').first().text()
      const coverCharge = this.parsePrice(priceText)

      // Description/excerpt
      const description = this.cleanText(
        $el.find('.eventlist-excerpt, .summary-excerpt, .event-description').first().text()
      )
      ...
      console.error('[Iron Horse] Error parsing event element:', error)
      return null
    }
  }
  ...
  private extractFromScripts(html: string): ScrapedEvent[] {
    ...
    if (scriptMatch && scriptMatch[1]) {
      try {
        ...
        if (context.collection?.items) {
          for (const item of context.collection.items) {
            const event = this.parseSquarespaceItem(item)
            if (event) events.push(event)
          }
        }
      } catch {
    ...
    // Also try looking for LD+JSON structured data
  ...
  private parseSquarespaceItem(item: Record<string, unknown>): ScrapedEvent | null {
    try {
  ...
  private generateEventId(title: string, date: Date): string {
    ...
    return `iron-horse-${dateStr}-${titleSlug}`
  }
}
