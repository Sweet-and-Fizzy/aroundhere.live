...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    // Assign unassigned venues to "Other" region
    if (unassigned.length > 0) {
      // Find or create the "Other" region (manual, not auto-generated)
      let otherRegion = await prisma.region.findUnique({
        where: { slug: 'other' },
      })
      if (!otherRegion) {
        otherRegion = await prisma.region.create({
          data: {
            name: 'Other',
            slug: 'other',
        ...
        changes.push('Created "Other" region for outlier venues')
      }

      const unassignedIds = unassigned.map(v => v.id)
      await prisma.venue.updateMany({
        where: { id: { in: unassignedIds } },
        data: { regionId: otherRegion.id },
      })
      await prisma.event.updateMany({
        where: { venueId: { in: unassignedIds } },
        data: { regionId: otherRegion.id },
      })
      changes.push(`Assigned ${unassigned.length} venues to "Other" region`)
    }
        ...
        [
          {
            ...
            text: {
              ...
              text: [
                ...
                `• Clustered: ${stats.clusteredVenues}`,
